...
import { isArrayVariableFirstCharacter, replaceVariableNameInFormula, replaceVariableNameInText } from '../src/data/VariableUtils';
...
await test('HoldVariable', async () => {
	...
	await test('Variable use and replace', async () => {
		...
		function generateAllNameVariations(noPrefixName: string, ...namesToExclude: string[]): string[] {
			const names = [];
		...
		function generateTestCases(variableNameBefore: string) {
			return baseTestCases.map(testCase => testCase.replace('~', variableNameBefore));
		}

		const runBigTest = async (noPrefixName: string, variableType: string, noPrefixNewName: string) => {
			...
			for (const badName of generateAllNameVariations(noPrefixName, name, name + suffix)) {
			...
			await test(`Variable ${variable.name.newValue} is used in formula/text`, async () => {
				for (const testCase of positiveTestCase) {
					await test(`Positive: '${testCase}'`, async () => {
						...
						assert.strictEqual(true, variable.isUsedInText(`$$ $${testCase}$ $$`));
						assert.strictEqual(true, variable.isUsedInText(`$$ $_MyX$ $_MyX * ${testCase} + _MyY$ $$`));
					})
				}
				...
				for (const testCase of negativeTestCases) {
					await test(`Negative: '${testCase}'`, async () => {
						...
						assert.strictEqual(false, variable.isUsedInText(`$$ $${testCase}$ $$`));
						assert.strictEqual(false, variable.isUsedInText(`$$ $_MyX$ $_MyX * ${testCase} + _MyY$ $$`));
					})
				}
			});
			await test(`Variable ${variable.name.newValue} renamed to ${newName}`, async () => {
				for (let i = 0; i < positiveTestCase.length; i++) {
					...
					await test(`In formulas: '${before}' to '${after}`, async () => {
						assert.strictEqual(after, replaceVariableNameInFormula(before, name, newName));
					});

					await test(`In texts: '${before}' to '${after}`, async () => {
						assert.strictEqual(replaceVariableNameInText(`$${before}$`, name, newName), `$${after}$`);
						assert.strictEqual(replaceVariableNameInText(`$$ $${before}+${before}$ $$`, name, newName), `$$ $${after}+${after}$ $$`);
						assert.strictEqual(replaceVariableNameInText(`$$ ${before} $${before}$ $$`, name, newName), `$$ ${before} $${after}$ $$`);
						assert.strictEqual(replaceVariableNameInText(`$$ $_MyX$ $_MyX * ${before} + _MyY$ $$`, name, newName), `$$ $_MyX$ $_MyX * ${after} + _MyY$ $$`);
					});

					await test(`In texts: Keep '${before}' when expression not between dollar signs`, async () => {
						assert.strictEqual(replaceVariableNameInText(before, name, newName), before);
						assert.strictEqual(replaceVariableNameInText(`some text ${before} some text`, name, newName), `some text ${before} some text`);
						assert.strictEqual(replaceVariableNameInText(`$$${before}$$`, name, newName), `$$${before}$$`);
						assert.strictEqual(replaceVariableNameInText(`$$ ${before} $$`, name, newName), `$$ ${before} $$`);
					})
				}
				...
				for (const testCase of negativeTestCases) {
					await test(`No change: '${testCase}'`, async () => {
						assert.strictEqual(replaceVariableNameInFormula(testCase, name, newName), testCase);
						assert.strictEqual(replaceVariableNameInText(`$${testCase}$`, name, newName), `$${testCase}$`);
						assert.strictEqual(replaceVariableNameInText(`$$ $${testCase}$ $$`, name, newName), `$$ $${testCase}$ $$`);
						assert.strictEqual(replaceVariableNameInText(`$$ $_MyX$ $_MyX * ${testCase} + _MyY$ $$`, name, newName), `$$ $_MyX$ $_MyX * ${testCase} + _MyY$ $$`);
					})
				}
			});
		};
		...
		await test(`Rename correctly handles all other variants of similar variable`, async () => {
			...
			const getCase = function (changedVariable: HoldVariable, newName: string, wrapCharacter: string) {
				return variables
