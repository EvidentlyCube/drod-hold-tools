...
const Columns: SortableTableColumn<HoldCharacter>[] = [
	{
		...
		filter: (character, filter) =>
			filterString(character.id.toString(), filter),
		filterDebounce: 500,
	},
	{
		id: "type",
		...
		sort: (isAsc, l, r) =>
			sortCompareString(isAsc, l.$baseTypeName, r.$baseTypeName),
		filter: (character, filter) =>
			filterString(character.$baseTypeName, filter),
		filterDebounce: 500,
	},
	{
		id: "avatar",
		...
		render: character => (
			<div className="is-flex is-gap-1 is-align-items-center">
				<SwapDataButton
					hold={character.$hold}
					dataSource={character.avatarDataId}
					formats={[DataFormat.PNG, DataFormat.JPG, DataFormat.BMP]}
				/>
				<DataRefViewByIdDynamic
					hold={character.$hold}
					dataIdSource={character.avatarDataId}
					showName={true}
				/>
			</div>
		),
		...
		filter: (character, filter) =>
			filterDataFormat(character.$avatarData?.details.newValue.format, filter),
	},
	{
		id: "tiles",
		displayName: "Tiles",
		...
		render: character => (
			<div className="is-flex is-gap-1 is-align-items-center">
				<SwapDataButton
					hold={character.$hold}
					dataSource={character.tilesDataId}
					formats={[DataFormat.PNG, DataFormat.JPG, DataFormat.BMP]}
				/>
				<DataRefViewByIdDynamic
					hold={character.$hold}
					dataIdSource={character.tilesDataId}
					showName={true}
				/>
			</div>
		),
	...
	},
	{
		...
		filter: (character, filter) =>
			filterString(character.name.newValue, filter),
		filterDebounce: 500,
	},
	{
		id: "uses",
	...
	if (!hold.version.data.isSupported) {
		return (
			<HoldVersionLimitationWarning
				warnings={["Custom characters were implemented in The City Beneath."]}
			/>
		);
	}
