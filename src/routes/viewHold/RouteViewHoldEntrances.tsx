...
import type { HoldEntrance } from "../../data/datatypes/HoldEntrance";
...
const ShowDescriptionTransformer = (value: string) => parseInt(value, 10);
...
const Columns: SortableTableColumn<HoldEntrance>[] = [
	...
	},
	{
		...
		sort: (isAsc, left, right) =>
			sortCompareString(
				isAsc,
				left.$level.name.newValue,
				right.$level.name.newValue,
			),
		filter: (entrance, filter) =>
			filterString(entrance.$level.name.newValue, filter),
		filterDebounce: 500,
	},
	{
		id: "show-description",
	...
	},
	{
		...
		widthPercent: 30,
		render: entrance => (
			<DrodTextEditor text={entrance.description} tag="textarea" />
		),
		sort: (isAsc, l, r) =>
			sortCompareString(isAsc, l.description.newValue, r.description.newValue),
		filter: (entrance, filter) =>
			filterString(entrance.description.newValue, filter),
		filterDebounce: 500,
	},
];
...
export default function RouteViewHoldEntrances() {
	...
	if (!hold.version.entrance.canHideDescription) {
		...
		warnings.push(
			"Controlling whether to show description not supported; Show Description column removed.",
		);
