...
import type { HoldWorldMap } from "../../data/datatypes/HoldWorldMap";
...
const Columns: SortableTableColumn<HoldWorldMap>[] = [
	...
	},
	{
		...
		filter: (worldMap, filter) =>
			filterString(worldMap.orderIndex.toString(), filter),
		filterDebounce: 500,
	},
	{
		id: "image",
		...
		filter: (worldMap, filter) =>
			filterDataFormat(worldMap.$data?.details.newValue.format, filter),
	},
	{
		id: "name",
		displayName: "Name",
	...
	if (!hold.version.worldMaps.isSupported) {
		return (
			<HoldVersionLimitationWarning
				warnings={["World Maps were implemented in The Second Sky"]}
			/>
		);
	}
