...
export default function SortableTable<TData extends SortableTableDataWithId>(
	props: Props<TData>,
) {
	const { tableId, columns, rows, className, pageSize } = props;

	const { sortBy, sortAsc, onSort } = useSortableTableSort(
		columns[0].id ?? "id",
	);
	const { visibleColumns, hiddenColumns, toggleHiddenColumn } =
		useSortableTableHiddenColumns(columns, tableId);
	const { filterableColumns, columnFilters, setColumnFilter } =
		useSortableTableFilterColumns(columns, tableId);
	const [page, setPage] = useLocalStorageState(`${tableId}-page`, {
		defaultValue: 0,
	});
		...
		<table className={className}>
			...
			<tbody>
				{paginatedRows.map(row => (
					<Row key={row.id} columns={visibleColumns} data={row} />
				))}
				{paginatedRows.length === 0 && (
					<tr>
						<td colSpan={visibleColumns.length}>
							<div className="container has-text-centered">Empty Table</div>
						</td>
					</tr>
				)}
			</tbody>
...
function Row<TData extends SortableTableDataWithId>({
	columns,
	data,
}: RowProps<TData>) {
	return (
		<tr>
			{columns.map(column => {
				...
				return (
					<td key={column.id} className={column.className} style={style}>
						{column.render(data)}
					</td>
				);
			})}
