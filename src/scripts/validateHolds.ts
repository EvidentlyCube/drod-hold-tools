...
const LOG_FIELD = (name: string, value: string) =>
	console.log(ANSI_BOLD(` - ${name}: `) + ANSI_GRAY(value));
...
void (async () => {
	...
	holds.sort((l, r) =>
		l.toLocaleLowerCase().localeCompare(r.toLocaleLowerCase()),
	);
	...
	for (const holdFullPath of holds) {
		...
		const result = await readHold(
			1,
			{ data: new Uint8Array(holdBuffer) },
			[],
			(s, l, t) => {
		...
		writeFileSync(
			`${__dirname}/validateHolds.log.imported`,
			truncateHold(originalHoldXml),
			"utf-8",
		);
		...
		if (causedBy instanceof XmlToHoldError) {
			...
			writeFileSync(
				`${__dirname}/validateHolds.log.exported`,
				truncateHold(await holdToXml(causedBy.hold)),
				"utf-8",
			);

			LOG_FIELD("Hold scripts in:", `${__dirname}/validateHolds.log.exported`);
			writeFileSync(
				`${__dirname}/validateHolds.log.scripts`,
				getHoldCommandsExport(causedBy.hold),
			);
			...
			if (rootError && rootError instanceof DiffXmlError) {
				for (const detail of rootError.details) {
					if (detail.type === "to-store") {
						LOG_FIELD(
							`${detail.name} in:`,
							`${__dirname}/validateHolds.log.${detail.fileSuffix}`,
						);
						writeFileSync(
							`${__dirname}/validateHolds.log.${detail.fileSuffix}`,
							detail.value,
...
function truncateHold(holdXml: string) {
	return holdXml
		...
		.replace(
			/RawData='(.*?)'/g,
			(_, value) => `RawData='[TRUNCATED ${value.length} bytes]'`,
		)
		.replace(
			/ExtraVars='(.*?)'/g,
			(_, value) => `ExtraVars='[TRUNCATED ${value.length} bytes]'`,
		)
		.replace(
			/Squares='(.*?)'/g,
			(_, value) => `Squares='[TRUNCATED ${value.length} bytes]'`,
		)
		.replace(
			/Message='(.*?)'/g,
			(_, value) => `Message='[TRUNCATED ${value.length} bytes]'`,
		)
		.replace(
			/TileLights='(.*?)'/g,
			(_, value) => `TileLights='[TRUNCATED ${value.length} bytes]'`,
		);
}
