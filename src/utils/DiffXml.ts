import { type PackedVar, PackedVarType } from "../data/PackedVars";
...
async function compareElement(
	...
	const originalAttributeNames = Array.from(original.attributes).map(
		node => node.name,
	);
	const generatedAttributeNames = Array.from(generated.attributes).map(
		node => node.name,
	);

	// Special hack for v100 because some holds there had this attribute in the wrong place
	if (
		original.tagName === "Demos"
		&& original.attributes[0].name === "DemoID"
	) {
	...
	const originalUniqueAttributes = diffArraysOneWay(
	...
	const generatedUniqueAttributes = diffArraysOneWay(
		generatedAttributeNames,
		originalAttributeNames,
	);
	...
	for (let i = 0; i < original.attributes.length; i++) {
		...
		assertNotNull(originalAttr, "Unexpected undefined for attribute value");
		assertNotNull(generatedAttr, "Unexpected undefined for attribute value");
		...
		if (originalAttr.name !== generatedAttr.name) {
			...
			if (originalAttr.name === "ExtraVars") {
				...
				extraDetails.push({
					...
					value: Array.from(base64ToUint8(generatedAttr.value))
				...
				extraDetails.push({
					...
					value: Array.from(base64ToUint8(originalAttr.value))
						.map(x => x.toString(16).padStart(2, "0"))
						.join(" "),
				...
				const length = Math.max(
					originalExtraVars.length,
					generatedExtraVars.length,
				);
...
function previewPackedVar(val: PackedVar) {
	const result =
		Array.isArray(val.value) && val.type === PackedVarType.ByteBuffer
